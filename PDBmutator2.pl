...
my $inputFrame = $mw->Frame(	-label => "DATA INPUT - variants",
	...
	my $allCheck = $inputFrame->Checkbutton( -text => "create all possible variants",
						-variable=>\$ALL
						);
	my $alnCheck = $inputFrame->Checkbutton( -text => "create all variants from .aln file (not available)",
						-variable=>\$ALN
						);

    my $alnFrame = $inputFrame->Frame();
		my $alnLabel = $alnFrame->Label(-text=>"file (e.g. 1cdw_align) : ");
...
my $pdbFrame = $mw->Frame();
	my $fileFrame = $pdbFrame->Frame();
		my $fileLabel = $fileFrame->Label(-text=>"pdb ID (e.g. 1cdw_bound) : ");
	...
	my $startFrame = $pdbFrame->Frame();
      ...
      my $endFrame = $pdbFrame->Frame();
       ...
       my $chainrFrame = $pdbFrame->Frame();
		my $chainrLabel = $chainrFrame->Label(-text=>"reverse complement chain ID: ");
		my $chainrEntry = $chainrFrame->Entry(-borderwidth => 2,
					-relief => "groove",
					-textvariable=>\$chainrID
					);
       my $startrFrame = $pdbFrame->Frame();
		my $startrLabel = $startrFrame->Label(-text=>"start position on reverse complement chain: ");
...
# Buttons
my $execButton = $mw -> Button(-text => "execute DNA mutations", 
				-command => \&execute
...
print "NOTE: reference PDB sequence assumed in first row and variant sequences underneath\n";
print "NOTE: reference PDB sequence MAY NOT have gap symbols (i.e. insertions)\n";
print "NOTE: the variant sequences MAY have gap symbols (i.e. deletions)\n\n";

...
my $inputFrame = $mw->Frame(	-label => "DATA INPUT - variants",
	...
	my $allCheck = $inputFrame->Checkbutton( -text => "create all possible variants (potentially huge)",
						-variable=>\$ALL
						);
...
my $pdbFrame = $mw->Frame();
	my $fileFrame = $pdbFrame->Frame();
		my $fileLabel = $fileFrame->Label(-text=>"pdb ID (e.g. 1ubq) : ");
...
# Buttons
my $execButton = $mw -> Button(-text => "execute AA replacements", 
				-command => \&execute
